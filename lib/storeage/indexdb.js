...
class IndexDBTable extends abstract_db_1.AbstractTable {
    ...
    deserialize(valueNew) {
        if ((0, samtools_1.isString)(valueNew) && valueNew.toLowerCase().startsWith('function(')) {
        ...
        try {
            ...
            return new Promise((reslove) => {
                res.onsuccess = (event) => {
                    ...
                    if ((0, samtools_1.isEmpty)(res.result))
...
class IndexDbStorage extends abstract_db_1.AbstractDB {
    ...
    table(name) {
        ...
        return new IndexDBTable(this.config, this.db.transaction([key], "readwrite").objectStore(key), name);
    }
}
