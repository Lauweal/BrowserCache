...
class AbstractDB {
    ...
    log(level, action, ...message) {
        if (!this.config.debug)
            return;
        if (level === 'error') {
            samtools_1.log.error(`[DATA_${this.config.name}]:${action}${this.version}`, ...message);
        }
        if (level === 'info') {
            samtools_1.log.info(`[DATA_${this.config.name}]:${action}${this.version}`, ...message);
        }
        if (level === 'waring') {
            samtools_1.log.warn(`[DATA_${this.config.name}]:${action}${this.version}`, ...message);
        }
    }
    get version() {
